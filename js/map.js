...
function renderMap(pos, showCurrentPos) {
	...
	if (showCurrentPos) {
		...
		marker.setIcon(new qq.maps.MarkerImage('images/car1.png', null, null,
...
function showMarker(markers){
	...
	for(var i=0;i<length;i++){
		var marker = new qq.maps.Marker({
					map : map,
					icon : new qq.maps.MarkerImage(markers[i].img ||'images/icon_adr.png', null, null,
							null, new qq.maps.Size(25, 30)),
	...
	drivingService.setComplete(function(result) {
		...
		var addressConvertor = new qq.maps.Geocoder({
			...
			complete: function(result) {
				...
				//改变回到中心控件的位置
				$(".icon_restore").css({margin:"0px 5px 100px 0px"});
			},
			//若服务请求失败，则运行以下函数
			error: function() {
				alert("出错了，请输入正确的经纬度！！！");
